...
class BatchGenerator(Sequence):
    ...
    def _change_obj_position(self, y_batch, anchors_map, idx, box, iou):
        ...
        y_batch[idx[0], idx[1], idx[2], idx[3], 4] = 0
        y_batch[idx[0], idx[1], idx[2], idx[3], 5] = 1
        y_batch[idx[0], idx[1], idx[2], idx[3], 6] = 0
...
class SSLBatchGenerator(Sequence):
    def __init__(self, lab_data, pseudo_lab_data, config, shuffle=True, jitter=True):

        self._lab_data = lab_data
        self._pseudo_lab_data = pseudo_lab_data
        self._config = config
        ...
        often = lambda aug: iaa.Sometimes(0.6, aug)

        # Define our sequence of augmentation steps that will be applied to every image
        # All augmenters with per_channel=0.5 will sample one value _per image_
        # in 50% of all cases. In all other cases they will sample new values
        # _per channel_.
        self._weak_aug_pipe = iaa.Sequential(
            [
                # apply the following augmenters to most images
                iaa.Fliplr(0.5),  # horizontally flip 50% of all images
                iaa.Flipud(0.0),  # vertically flip 20% of all images
        ...
        self._strong_aug_pipe = iaa.Sequential(
            [
                # apply the following augmenters to most images
                iaa.Fliplr(0.5),  # horizontally flip 50% of all images
                iaa.Flipud(0.0),  # vertically flip 20% of all images
                ...
                iaa.SomeOf((2, 5),
                           [
                               often(iaa.Superpixels(p_replace=(0, 1.0), n_segments=(20, 200))),
                               iaa.OneOf([
                                   iaa.GaussianBlur((0, 3.0)),  # blur images with a sigma between 0 and 3.0
                                   iaa.AverageBlur(k=(2, 7)),
                                   # blur image using local means (kernel sizes between 2 and 7)
        ...
        if shuffle:
            np.random.shuffle(self._lab_data)
            np.random.shuffle(self._pseudo_lab_data)
    
    def __len__(self):
        return min(int(np.ceil(float(len(self._lab_data) / self._config['LAB_BATCH_SIZE']))), int(np.ceil(float(len(self._pseudo_lab_data) / self._config['PSEUDO_LAB_BATCH_SIZE']))))

    def num_classes(self):
        return len(self._config['LABELS'])

    def size_lab(self):
        return len(self._lab_data)
    
    def size_pseudo_lab(self):
        return len(self._pseudo_lab_data)

    def load_lab_data(self, i):
      return self._lab_data[i]
    
    def load_pseudo_lab_data(self, i):
      return self._pseudo_lab_data[i]

    def load_annotation(self, i):
        annots = []

        for obj in self._lab_data[i]['object']:
            annot = [obj['xmin'], obj['ymin'], obj['xmax'], obj['ymax'], self._config['LABELS'].index(obj['name'])]
            annots += [annot]
    ...
    def load_pseudo_annotation(self, i):
        annots = []

        for obj in self._pseudo_lab_data[i]['object']:
            annot = [obj['xmin'], obj['ymin'], obj['xmax'], obj['ymax'], self._config['LABELS'].index(obj['name'])]
            annots += [annot]
    ...
    def load_image(self, i):
        if self._config['IMAGE_C'] == 1:
            image = cv2.imread(self._lab_data[i]['filename'], cv2.IMREAD_GRAYSCALE)
            image = image[..., np.newaxis]
        elif self._config['IMAGE_C'] == 3:
            image = cv2.imread(self._lab_data[i]['filename'])
        else:
            raise ValueError("Invalid number of image channels.")
        return image
    
    def load_pseudo_image(self, i):
        if self._config['IMAGE_C'] == 1:
            image = cv2.imread(self._pseudo_lab_data[i]['filename'], cv2.IMREAD_GRAYSCALE)
            image = image[..., np.newaxis]
        elif self._config['IMAGE_C'] == 3:
            image = cv2.imread(self._pseudo_lab_data[i]['filename'])
        else:
            raise ValueError("Invalid number of image channels.")
        return image
    
    def __getitem__(self, idx):
        l_bound_lab = idx * self._config['LAB_BATCH_SIZE']
        r_bound_lab = (idx + 1) * self._config['LAB_BATCH_SIZE']

        l_bound_pseudo_lab = idx * self._config['PSEUDO_LAB_BATCH_SIZE']
        r_bound_pseudo_lab = (idx + 1) * self._config['PSEUDO_LAB_BATCH_SIZE']

        if r_bound_lab > len(self._lab_data):
            r_bound_lab = len(self._lab_data)
            l_bound_lab = r_bound_lab - self._config['LAB_BATCH_SIZE']
        
        if r_bound_pseudo_lab > len(self._pseudo_lab_data):
            r_bound_pseudo_lab = len(self._pseudo_lab_data)
            l_bound_pseudo_lab = r_bound_pseudo_lab - self._config['PSEUDO_LAB_BATCH_SIZE']

        batch_data = self._lab_data[l_bound_lab:r_bound_lab] + self._pseudo_lab_data[l_bound_pseudo_lab:r_bound_pseudo_lab]

        instance_count = 0
        x_batch = np.zeros((self._config['LAB_BATCH_SIZE'] + self._config['PSEUDO_LAB_BATCH_SIZE'],
                            self._config['IMAGE_H'], self._config['IMAGE_W'], self._config['IMAGE_C']))  # input images

        y_batch = np.zeros((self._config['LAB_BATCH_SIZE'] + self._config['PSEUDO_LAB_BATCH_SIZE'], self._config['GRID_H'], self._config['GRID_W'],
                            self._config['BOX'], 4 + 3 + len(self._config['LABELS'])))  # desired network output

        anchors_populated_map = np.zeros((self._config['LAB_BATCH_SIZE'] + self._config['PSEUDO_LAB_BATCH_SIZE'],
                                          self._config['GRID_H'], self._config['GRID_W'], self._config['BOX']))
        

        for train_instance in batch_data:
            p_labelled = train_instance['pseudo_labelled']
            # augment input image and fix object's position and size
            img, all_objs = self.aug_image(train_instance, jitter=self._jitter)
    ...
    def _change_obj_position(self, y_batch, anchors_map, idx, box, p_labelled, obj_pl_conf, iou):
        ...
        y_batch[idx[0], idx[1], idx[2], idx[3], 5] = obj_pl_conf
        y_batch[idx[0], idx[1], idx[2], idx[3], 6] = 1-obj_pl_conf
        y_batch[idx[0], idx[1], idx[2], idx[3], 7:] = 0  # clear old values
        y_batch[idx[0], idx[1], idx[2], idx[3], 4 + 3 + idx[4]] = 1
        shifted_box = BoundBox(0, 0, bkp_box[2], bkp_box[3])
    ...
    def aug_image(self, train_instance, jitter):
        ...
        if jitter:
            ...
            if train_instance['pseudo_labelled']:
                ...
                image, bbs = self._weak_aug_pipe(image=image, bounding_boxes=bbs)
